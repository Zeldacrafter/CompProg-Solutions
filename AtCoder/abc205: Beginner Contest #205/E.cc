...
int main() {
    ...
    vector<mod> facs(totalW + totalB + 1);
    facs[0] = 1;
    FOR(i, 1, SZ(facs))
        facs[i] = i*facs[i - 1];
    vector<mod> invs(SZ(facs));
    F0R(i, SZ(invs)) invs[i] = 1/facs[i];

    auto ch = [&](int n, int k) {
        if(n < 0 || k < 0 || k > n) return mod(0);
        return facs[n]*invs[k]*invs[n-k];
    };

    cout << ch(totalW + totalB, totalW) - ch(totalW + totalB, totalB + k + 1) << endl;
