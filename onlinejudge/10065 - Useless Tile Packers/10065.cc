...
int dot(pt a, pt b) { return a.xx * b.xx + a.yy * b.yy; }
...
double area(vector<pt>& pts) {
    double res = 0;
    F0R(i, SZ(pts))
        res += cross(pts[i], pts[(i+1)%SZ(pts)]);
    return abs(res) / 2;
}
...
void solve(int n) {
    ...
    double small = area(pts);

    vector<pt> hull = convexHull(pts);
    hull.pop_back();

    double big = area(hull);

    cout << "Wasted Space = " << setprecision(2) << fixed << 100 - small/big*100 << " %\n\n";
}
