...
array<int, 8> xd{1, 1, 1, -1, -1, -1, 0, 0};
array<int, 8> yd{0, 1, -1, 0, 1, -1, 1, -1};
...
void solve() {
  ...
  while (not q.empty()) {
    ...
    if (cx == x and cy == y) {
      ...
      out.eb(x, y);
      do {
        auto [xx, yy] = par[{cx, cy}];
        swap(cx, xx); swap(cy, yy);
        out.eb(cx, cy);
    ...
    F0R (i, 8) {
      ...
      if (min(xx, yy) >= 0 and max(xx, yy) < w and not seen.contains({xx, yy}) and mm[yy][xx] == 'W') {
        seen.emplace(xx, yy);
        q.emplace(xx, yy);
        par[{xx, yy}] = {cx, cy};
      }
