...
void solve(int tc) {
    ...
    stringstream sCin(inp);

    ii last(-1, -1);
    int x, y; char c;

    sCin >> last.fi >> c >> last.se;
    last *= 2;
    seen.insert(last);

    while(sCin >> x >> c >> y) {
      x *= 2; y *= 2;
      assert(abs(x - last.fi) <= 2 && abs(y - last.se) <= 2);

      int dx = x > last.fi ? 1 : (x < last.fi ? -1 : 0);
      int dy = y > last.se ? 1 : (y < last.se ? -1 : 0);
