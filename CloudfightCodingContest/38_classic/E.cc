...
void solve() {
  ...
  while (not q.empty()) {
    ...
    q.pop();
    F0R (i, 4) {
      int xx = cx + xd[i], yy = cy + yd[i];
      if (min(xx, yy) >= 0 and max(xx, yy) < w and not seen.contains({xx, yy})) {
        seen.emplace(xx, yy);
        if (mm[yy][xx] == 'W') {
          border.emplace(xx, yy);
        } else {
          q.emplace(xx, yy);
        }
      }
  ...
  vii out;
  auto [xxx, yyy] = *border.begin();
  auto dfs = y_combinator([&](auto self, int x, int y) -> bool {
    // cout << dvary(x, y) << endl;
    out.eb(x, y);
    border -= {x, y};
    if (border.empty() and abs(x - xxx) <= 1 and abs(y - yyy) <= 1) return true;
    F0R (i, 8) {
      int xx = x + xdd[i], yy = y + ydd[i];
      if (border.contains({xx, yy})) {
        if (self(xx, yy)) return true;
      }
    }
    border += {x, y};
    out.pop_back();
  ...
  if (not dfs(xxx, yyy)) {
    cout << dvar(sx, sy) << endl;
    for (auto [x, y] : border) {
      mm[y][x] = 'O';
    }
    cout << dvarc(xxx + 1, yyy + 1) << endl;
    cout << pp(mm, "\n") << endl;
    assert(false);
  }
  cout << pp(out, " ", ",") << endl;
