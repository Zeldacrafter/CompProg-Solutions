...
vector<pt> convexHull(vector<pt>& pts) {
  ...
  FOR (i, 1, n) {
    ...
    if (i == n - 1 || !cw(pts[0], pts[n - 1], pts[i])) {
      while (SZ(up) > 1 &&
             !cw(up[SZ(up) - 2], up[SZ(up) - 1], pts[i]))
        up.pop_back();
      up.eb(pts[i]);
    }
    if (i == n - 1 || !ccw(pts[0], pts[n - 1], pts[i])) {
      while (SZ(down) > 1 &&
             !ccw(down[SZ(down) - 2], down[SZ(down) - 1], pts[i]))
        down.pop_back();
      down.eb(pts[i]);
    }
  }
...
vector<string> mm, mmm;
int w;

array<int, 8> xdd{1, 0, 0, -1, -1, -1, 1, 1};
array<int, 8> ydd{0, 1, -1, 0, 1, -1, 1, -1};

array<int, 4> xd{1, 0, -1, 0};
array<int, 4> yd{0, 1, 0, -1};
...
void solve() {
  ...
  while (not q.empty()) {
    ...
    q.pop();
    F0R (i, 4) {
      int xxx = cx + xd[i], yyy = cy + yd[i];
      if (min(xxx, yyy) >= 0 and max(xxx, yyy) < w and not seen.contains({xxx, yyy})) {
        seen.emplace(xxx, yyy);
        if (mm[yyy][xxx] == 'W') {
          border.emplace(xxx, yyy);
        } else {
          q.emplace(xxx, yyy);
        }
      }
  ...
  dout << dvar(sx, sy, border) << endl;
  for (auto [x, y] : border) mmm[y][x] = 'O';
  vector<pt> pts;
  for (auto [x, y] : border) pts.eb(x, y);
  pts = convexHull(pts);
  vii hull;
  for (auto p : pts) hull.eb(p.xx, p.yy);
  for (auto [x, y] : hull) mmm[y][x] = 'X';
  // cout << dvarc(hull) << endl;

  auto bfs = [&](ii s, ii e) -> vii {
    set<ii> seen;
    map<ii, ii> par;
    queue<ii> q;
    seen += s;
    ...
    while (not q.empty()) {
      ...
      auto [cx, cy] = q.front();
      q.pop();
      if (cx == e.fi and cy == e.se) {
        // cout << dvarr(par) << endl;
        vii out;
        out.eb(cx, cy);
        while (cx != s.fi or cy != s.se) {
          auto [xxx, yyy] = par[{cx, cy}];
          swap(cx, xxx); swap(cy, yyy);
          out.eb(cx, cy);
        }
        out.pop_back();
        reverse(ALL(out));
      ...
      F0R (i, 8) {
        int xxx = cx + xdd[i], yyy = cy + ydd[i];
        if (min(xxx, yyy) >= 0 and max(xxx, yyy) < w and not seen.contains({xxx, yyy}) and mm[yyy][xxx] == 'W') {
          seen.emplace(xxx, yyy);
          q.emplace(xxx, yyy);
          par[{xxx, yyy}] = {cx, cy};
        }
  ...
  // cout << dvar(hull) << endl;
  F0R (i, SZ(hull) - 1) {
    auto part = bfs(hull[i], hull[i + 1]);
    // cout << dvar(part) << endl;
    out.insert(out.end(), ALL(part));
  }
  cout << pp(out, " ", ",") << endl;
}
