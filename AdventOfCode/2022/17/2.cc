...
void solve() {
    // last rows, type, dirI -> amt, maxY
    map<tuple<set<pll>, ll, ll>, 
        pair<ll, ll>> patterns;
    ...
    for(ll amt = 0, dirI = 0; amt < GOAL; ++amt) {
        ...
        while(true) {
            ...
            if(ok) coord = newCoord;
                ...
                // Are last 25 (?) lines same as something already known?
                // NOTE: This does not work for every input..
                set<pll> last25;
                for (auto [x, y] : seen)
                    if(maxY - y <= 25) last25.emplace(x, maxY - y);

                auto state = mt(last25, type, dirI);
                if(patterns.count(state)) {
                    auto [old_amt, old_maxY] = patterns[state];
                    ll dy = maxY - old_maxY;
                    ll dAmt = amt - old_amt;
                    ll reps = (GOAL - amt) / dAmt;
                    yExtra += reps * dy;
                    amt += reps * dAmt;
                }
                patterns[state] = mp(amt, maxY);
                break;
            }
        }
    }

    cout << maxY + yExtra << endl;
}
