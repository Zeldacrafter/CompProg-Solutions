...
void solve() {
    ...
    auto cmp = [](ii a, ii b) { 
        return mp(max(a.fi, a.se), min(a.fi, a.se)) 
             > mp(max(b.fi, b.se), min(b.fi, b.se));
    };
    priority_queue<ii, vii, decltype(cmp)> scores(cmp);
    F0R(s1, 21) F0R(s2, 21) scores.emplace(s1, s2);
    ...
    ll win1 = 0, win2 = 0;
    while(SZ(scores)) {
        auto [s1, s2] = scores.top(); scores.pop();
        F0R(p1, 10) F0R(p2, 10) { // pos player 1, pos player 2
