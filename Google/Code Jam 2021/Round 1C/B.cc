...
int getDigitCnt(ll y) {
    int digitCnt = 0;
    for(int x = y; x; x /= 10) digitCnt++;
    return digitCnt;
}

ll findNext(ll y) {
    int digitCnt = getDigitCnt(y);

    ll best = numeric_limits<ll>::max();

    for(int digits = 1; 2*digits <= digitCnt ; ++digits) {

        ll leadingDig = y/pow10(digitCnt - digits);
        F0R(_, 1000) {
            ll leading = leadingDig;

            ll res = leading * pow10(digitCnt - digits);
            int added = digits;
            while(added < digitCnt) {
                int next = ++leading;
                int dCnt = getDigitCnt(next);
                added += dCnt;

                if(added > digitCnt) break;

                res += next*pow10(digitCnt - added);
            }
            if(added == digitCnt && res > y) {
                ckmin(best, res);
            }

            if(getDigitCnt(leadingDig) == getDigitCnt(leadingDig + 1)) {
                leadingDig++;
            } else {
                break;
            }
    ...
    if(best == numeric_limits<ll>::max()) {
        ckmin(best, findNext(pow10(digitCnt)));
    }
    return best;
}
