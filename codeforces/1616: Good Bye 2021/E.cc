...
struct FT {
  ...
  T query(int i, int j) { return query(j) - query(i); } // [l, r)
...
void solve() {
    ...
    // Buckets of letter positions. Last occurrance first
    vvi letters(26);
    for(int i = n - 1; ~i; --i)
        letters[s[i] - 'a'].push_back(i);

    // Current position of letter that started at s[i]
    FT<ll> toPos(n + 1);
    F0R(i, n) {
        toPos.update(i, i);
        toPos.update(i + 1, -i);
    ...
    ll currMoves = 0, best = numeric_limits<ll>::max();
    F0R(pos, n) {
        // Find first smaller than t[pos], get to front
        F0R(i, t[pos] - 'a') {
            if(SZ(letters[i])) {
                ckmin(best, currMoves + toPos.query(letters[i].back() + 1) - pos);
            }
        }

        // Cannot make s[pos] == t[pos]? We are done.
        if(!SZ(letters[t[pos] - 'a'])) break;

        // Make s[pos] == t[pos]:

        // Move first occ of t[pos] in s to the front
        int toFront = toPos.query(letters[t[pos] - 'a'].back() + 1);
        toPos.update(0, 1);
        toPos.update(letters[t[pos] - 'a'].back() + 1, -1);
        letters[t[pos] - 'a'].pop_back();
        
        // We needed that many moves
        currMoves += toFront - pos;
    }

    cout << (best == numeric_limits<ll>::max() ? -1 : best) << endl;
}
