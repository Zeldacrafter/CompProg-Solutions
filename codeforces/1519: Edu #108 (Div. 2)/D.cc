...
int main() {
    ...
    F0R(mid, n) {
        ...
        FOR(len, 1, n) {
            ...
            curr -= a[mid - len]*b[mid - len] + a[mid + len]*b[mid + len];
            curr += a[mid + len]*b[mid - len] + a[mid - len]*b[mid + len];
            ckmax(res, curr);
        }
        curr = amt;
        FOR(len, 1, n) {
            if(mid + len >= n || mid - len + 1 < 0) break;
            curr -= a[mid - len + 1]*b[mid - len + 1] + a[mid + len]*b[mid + len];
            curr += a[mid + len]*b[mid - len + 1] + a[mid - len + 1]*b[mid + len];
            ckmax(res, curr);
        }
    }
    cout << res << endl;
