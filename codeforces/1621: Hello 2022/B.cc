...
void solve() {
    int n;
    cin >> n;
    vector<ll> ls(n), rs(n), cs(n);
    F0R(i, n) cin >> ls[i] >> rs[i] >> cs[i];

    ll mnL = 1e18, mnR = 1e18, mnLR = 1e18;
    ll l = 1e18, r = -1e18;
    F0R(i, n) {
        ...
        if(ls[i] == l) ckmin(mnL, cs[i]);
        if(rs[i] == r) ckmin(mnR, cs[i]);

        if(ls[i] == l && rs[i] == r) ckmin(mnLR, cs[i]);

        cout << min(mnL + mnR, mnLR) << endl;
    }
}
