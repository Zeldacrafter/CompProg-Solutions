...
vi seen;
vector<ll> path, treeSz;
bool found;

void dfsCycle(int v = 0, int p = -1) {
    seen[v] = true;
...
void dfsPath(int v = cycle1, int goal = cycle2) {
    seen[v] = true;
    if(!found)
        path.pb(v);
    ...
    for(int u : adj[v]) if((v != cycle1 || u != cycle2) && (v != cycle2 || u != cycle1) && !seen[u] && !found) {
        dfsPath(u, goal);
    }
...
int dfsSubtree(int v, int bad1, int bad2) {
    seen[v] = true;
    int sz = 1;
    for(int u : adj[v]) 
        if(!seen[u] && u != bad1 && u != bad2 && (v != cycle1 || u != cycle2) && (v != cycle2 || u != cycle1))
            sz += dfsSubtree(u, bad1, bad2);
    return sz;
}
...
void solve() {
    ...
    seen.assign(n, false);
    treeSz.assign(pLen, 0);
    F0R(i, pLen)
        treeSz[i] = dfsSubtree(path[i], !i ? -1 : path[i - 1], i == pLen - 1 ? -1 : path[i + 1]);

    vector<ll> pref(pLen, 0);
    F0R(i, pLen)
        pref[i] = treeSz[i] + (i ? pref[i - 1] : 0);

    ll res = (ll)n * (n - 1) / 2;
    F0R(i, pLen - 1)
        res += treeSz[i] * (pref[SZ(path) - 1] - pref[i]);
