...
vvi adj;
vector<ll> leafes, a, dp, totalPpl;
...
void dfs(int v = 0) {
    ...
    for(int u : adj[v]) {
        dfs(u);
        leafes[v] += leafes[u];
        totalPpl[v] += totalPpl[u];
        ckmax(mx, dp[u]);
        assert(totalPpl[u] <= leafes[u]*dp[u]);
    }

    totalPpl[v] += a[v];

    ll pplLeft = totalPpl[v] - leafes[v]*mx;

    dp[v] = mx;
    if(pplLeft > 0)
        dp[v] += (pplLeft + leafes[v] - 1) / leafes[v];
}
