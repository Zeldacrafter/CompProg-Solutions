...
void solve() {
    ...
    map<string, ll> amt;
    map<string, string> first3, last3;
    ll res = 0;
    ...
    F0R(i, n) {
        ...
        if(cin.peek() == ':') {
            ...
            amt[iden] = res = getOcc(val);
            first3[iden] = val.substr(0, 3);
            last3[iden] = val.substr(max(0, SZ(val) - 3), 3);
        } else {
            string val1, val2;
            cin >> val1 >> val1 >> val2 >> val2;

            amt[iden] = res = amt[val1] + amt[val2] + getOcc(last3[val1] + first3[val2]);
            first3[iden] = (first3[val1] + first3[val2]).substr(0, 3);

            string newS = last3[val1] + last3[val2];
            last3[iden] = newS.substr(max(0, SZ(newS) - 3), 3);
        }
    }
    cout << res << endl;
}
