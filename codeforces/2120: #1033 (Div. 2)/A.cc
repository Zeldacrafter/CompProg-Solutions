...
void solve() {
   ...
   ok |= a[0].se == a[1].se && a[1].se == a[2].se && a[0].fi + a[1].fi + a[2].fi == a[0].se;

   F0R(_, 3) {
       ok |= a[0].fi == a[1].fi + a[2].fi && a[1].se == a[2].se
           && a[0].fi == a[0].se + a[1].se;
       ok |= a[0].se == a[1].se + a[2].se && a[1].fi == a[2].fi
           && a[0].se == a[0].fi + a[1].fi;

       ii x = a[0], y = a[1], z = a[2];
       a[0] = y;
       a[1] = z;
       a[2] = x;
   }
...
int main() {
    ...
    cout << boolalpha;
    cout.imbue(locale(cout.getloc(), new yes_no));
