...
int main() {
    ...
    int end1 = getFurthest(0).fi;
    auto [end2, mxDist] = getFurthest(end1);

    vi dist1 = getDist(end1);
    vi dist2 = getDist(end2);

    F0R(i, n)
        cout << max({mxDist, dist1[i] + 1, dist2[i] + 1}) << endl;
}
