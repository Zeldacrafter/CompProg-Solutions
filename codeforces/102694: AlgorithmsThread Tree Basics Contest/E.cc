...
int main() {
    ...
    while(q--) {
        ...
        if(t == 1) {
            x--;
            ft.update(in[x], -ft.query(in[x], in[x] + 1));
            ft.update(in[x], log(y));
        } else {
            x--; y--;
            long double treeX = ft.query(in[x], out[x] + 1);
            long double treeY = ft.query(in[y], out[y] + 1);
            cout << fixed << setprecision(8) << min((long double)1e9, exp(treeX - treeY)) << endl;
        }
    }
}
