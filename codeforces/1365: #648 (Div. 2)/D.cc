...
bool ok(int r, int c){ return r >= 0 && r < rows && c >= 0 && c < cols; }
void solve() {
    cin >> rows >> cols;
    ...
    F0R(r, rows) 
        F0R(c, cols) {
            ...
            F0R(i, 4) {
                ...
                if(!ok(rr, cc) || a[rr][cc] == 'B') continue;
    ...
    if(a[rows - 1][cols - 1] == '#' && cnt) {
        cout << "No\n";
        return;
    }

    queue<ii> q;
    q.emplace(rows - 1, cols - 1);
    a[rows - 1][cols - 1] = '?';
    int found = 0;
    while(!q.empty()) {
        auto [r, c] = q.front();
        ...
        F0R(i, 4) {
            int rr = r + dx[i], cc = c + dy[i];
            if(!ok(rr, cc) || a[rr][cc] == '#' || a[rr][cc] == '?') continue;
