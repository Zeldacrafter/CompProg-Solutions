...
array<pair<ll, int>, 3> piles;
void query() {

    ll y = piles[2].fi*2 - piles[1].fi - piles[0].fi;
    cout << y << endl;
    ...
    if(pile == piles[0].se) {
        cout << piles[2].fi - piles[1].fi << endl;
    } else if(pile == piles[1].se) {
        cout << piles[2].fi - piles[0].fi << endl;
    } else {
        piles[2].fi += y;
        query();
    }
...
void solve() {
    ...
    cin >> piles[0].fi >> piles[1].fi >> piles[2].fi;
    sort(ALL(piles));
