...
void solve() {
    ...
    vii coords;
    F0R(i, n) F0R(j, m) {
        if(ckmax(mx, a[i][j])) coords.clear();
        if(mx == a[i][j]) coords.eb(i, j);
    }
    ...
    FOR(i, 1, SZ(coords)) {
        auto [r, c] = coords[i];
        auto [pr, pc] = coords[i - 1];

        if(swaps == 0 && c != pc) swaps = 1;
        else if(swaps == 1 && r != pr)  swaps = 2;
        else if(swaps == 2 && c != pc) ok = false;

        if(swaps == 2 && c != coords[0].se) ok = false;
    }
    ...
    FOR(i, 1, SZ(coords)) {
        ...
        if(swaps == 0 && r != pr) swaps = 1;
        else if(swaps == 1 && c != pc) ok2 = false;
    }
